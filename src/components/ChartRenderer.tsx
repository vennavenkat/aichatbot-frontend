...
const ChartRenderer: React.FC<ChartRendererProps> = ({ base64Image }) => {
  ...
  const generateChart = async () => {
    try {
      ...
      setGeneratedChart(res.data.imageBase64);
      setError(null);
    } catch (err) {
  ...
  // If base64Image is passed as prop, render only that
  if (base64Image) {
    return (
      <div className="mt-4">
        <img src={`data:image/png;base64,${base64Image}`} alt="Generated Chart" className="border rounded shadow" />
      </div>
    );
  }

  // Otherwise render the chart input form
  return (
